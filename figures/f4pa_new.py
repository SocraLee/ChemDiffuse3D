...
if __name__ == '__main__':
    ...
    print("\n" + "=" * 80)
    # ── Find and print slices comparing 'Ours' vs '3DRCAN' ──
    ours_df = df_plot[df_plot['Method'] == 'Ours'].copy()
    swin_df = df_plot[df_plot['Method'] == '3DRCAN'].copy()
    
    merged = pd.merge(ours_df, swin_df, on=['Sample_Idx', 'Slice_Idx'], suffixes=('_ours', '_swin'))
    merged['GT_Cells'] = merged['TP_ours'] + merged['FN_ours']
    merged['F1_diff'] = merged['F1_ours'] - merged['F1_swin']
    
    valid = merged[(merged['GT_Cells'] > 10) & (merged['F1_swin'] > 0.3) & (merged['F1_diff'] > 0.1)]
    
    if len(valid) > 0:
        valid = valid.sort_values('F1_ours', ascending=False)
        print(f"\n>>> 'Ours' vs '3DRCAN' SLICES (GT>10, Swin F1>0.3, Diff>0.1, Sorted by Ours F1) [Top 10] <<<")
        for _, row in valid.head(10).iterrows():
            print(f"Sample_Idx: {int(row['Sample_Idx']):<4} | Slice_Idx: {int(row['Slice_Idx']):<2} | Ours F1: {row['F1_ours']:.4f} | Swin F1: {row['F1_swin']:.4f} | Diff: {row['F1_diff']:.4f} | GT Cells: {int(row['GT_Cells'])}")
        print(f"{'=' * 80}")
    else:
        print(f"\n>>> No slices found matching criteria under {plot_desc} <<<")
