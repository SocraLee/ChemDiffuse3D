...
def load_decoder(decoder_type, decoder_path, device):
    """Load a post-diffusion decoder model."""
    if decoder_type == 'rcan':
        decoder = PureRCANDecoder().to(device)
    elif decoder_type == 'fused':
        decoder = FusedDecoder().to(device)
    else:
        return None
    ...
    if decoder_path and os.path.exists(decoder_path):
        ...
        logger.info(f"Loaded {decoder_type} decoder from {decoder_path}")
    else:
        logger.warning(f"Decoder path not found: {decoder_path}. Using random initialization.")

    decoder.eval()
    return decoder


def generate_and_decode(model, vae, latents_noise, cond_input, latents_scale, latents_bias,
    ...
    if decoder is not None and lr_image is not None:
        # Use post-diffusion decoder
...
def run_evaluation(args):
    ...
    decoder = None
    if args.decoder_type and args.decoder_type != 'none':
        decoder = load_decoder(args.decoder_type, args.decoder_path, device)
...
def main():
    ...
    # Decoder
    parser.add_argument("--decoder-type", type=str, default=None, choices=[None, 'none', 'rcan', 'fused'])
