...
def get_score_from_velocity(vt, xt, t, path_type="linear"):
    """
    Transform velocity prediction to score function.

    Args:
        vt: velocity model output, shape [batch_dim, ...]
        xt: current data point x_t, shape [batch_dim, ...]
        t: time tensor, shape [batch_dim,]
        path_type: interpolation path type
    """
    t = expand_t_like_x(t, xt)
    if path_type == "linear":
        alpha_t = 1 - t
        d_alpha_t = torch.ones_like(xt, device=xt.device) * -1
        sigma_t = t
...
def compute_diffusion(t_cur):
    """Compute diffusion coefficient for SDE sampler."""
    return 2 * t_cur
...
def euler_sampler(
    ...
    """
    Euler ODE sampler for deterministic sampling.

    Args:
        model: Diffusion model
        latents: Initial noise tensor
        cond_input: Tuple of (y, lr_img, task_id) conditioning inputs
        num_steps: Number of sampling steps
        heun: Whether to use Heun's method (2nd order)
        cfg_scale: Classifier-free guidance scale
        guidance_low: Lower bound for guidance application
        guidance_high: Upper bound for guidance application
        path_type: Interpolation path type
    """
    y, lr_img, task_id = cond_input
    if cfg_scale > 1.0:
        y_null = torch.zeros(y.shape, device=y.device)
    ...
    with torch.no_grad():
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):
            ...
            if cfg_scale > 1.0 and t_cur <= guidance_high and t_cur >= guidance_low:
                ...
                model_input = x_cur
                y_cur = y
            kwargs = dict(y=y_cur, lr_img=lr_img, task_id=task_id)
...
def euler_maruyama_sampler(
    ...
    """
    Euler-Maruyama SDE sampler for stochastic sampling.

    Adds Brownian noise at each step for improved sample diversity
    and quality compared to deterministic ODE sampling.
    ...
    t_steps = torch.linspace(1., 0, num_steps, dtype=torch.float64)
    x_next = latents.to(torch.float64)
    device = x_next.device

    with torch.no_grad():
        # Main SDE steps (all but last)
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-2], t_steps[1:-1])):
            ...
            kwargs = dict(
                y=y_cur,
                lr_image=lr_img.to(device=device, dtype=_dtype),
            )
            time_input = torch.ones(model_input.size(0)).to(device=device, dtype=torch.float64) * t_cur
